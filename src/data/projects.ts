...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: '基于 Spring AI 的 ReAct 工具调用 Agent',
    oneLiner:
      '手写 ReAct 执行循环的通用工具调用 Agent：自己维护 Agent 继承链和状态机，把工具调用、SSE 流式输出、多轮会话持久化串成一条真的能跑起来的链路，而不是套一个现成框架。',
    ...
    links: [
      {
        ...
        note: '含后端、Vue 3 前端，以及一个可独立运行的图片搜索 MCP Server 子模块',
      },
    ],
    scale: [
      { label: 'Java 文件', value: '88 个' },
      { label: '测试', value: '15 个文件 / 48 个用例' },
    ...
    metrics: [
      {
        value: '48',
        unit: '个',
        label: '活跃单元测试用例（15 个测试文件）',
        source: 'src/test/java/com/lcl/myaiagent/ 下各测试类的 @Test 方法统计',
        method:
          '其中 4 个 Agent 测试类共 33 个是纯单测（覆写 callLlm 返回受控响应，无外部依赖，2 秒内跑完）；其余依赖真实模型 Key 与 MySQL，因此整套跑不完，这也是没有 CI 的原因',
      },
      {
        value: '8',
        unit: '个',
        label: '注册给 Agent 的 @Tool 方法',
        source: 'config/ToolRegistration.java:44-53',
        method:
          '文件读写、网页搜索、网页抓取、资源下载、PDF 生成、询问人类、主动终止；另有 1 个终端命令工具被注释禁用，所以声明数是 9、运行期是 8',
      },
      {
        value: '4',
        unit: '层',
        label: 'Agent 继承链',
        source:
          'agent/BaseAgent.java:27 → agent/ReActAgent.java:18 → agent/ToolCallAgent.java:36 → agent/MyManus.java:18',
        method: 'BaseAgent 管状态机，ReActAgent 定义 think→act，ToolCallAgent 做工具编排，MyManus 是具体 Agent',
      },
      {
        value: '5',
        unit: '条',
        label: '任务终止路径',
        source:
          'ToolCallAgent.java:147-151 / 197-210、BaseAgent.java:106-109 / 245-251',
        method:
          '模型不再调用工具、主动调用终止工具、调用 askHuman、达到最大步数、循环检测触发，五条路都归到 FINISHED 状态',
      },
      {
        value: '3',
        unit: '次',
        label: '循环检测的强制终止阈值',
        source: 'agent/BaseAgent.java:49, 53, 303',
        method: '同一步骤重复出现 2 次即判定为卡住，累计 3 次后强制终止，避免无限循环烧 token',
      },
    ],
    architecture: [
      {
        ...
        nodes: [
          {
            name: 'AiController',
            note: '/ai/manus/chat 用 SseEmitter 推送，超时 5 分钟；/ai/love_app/* 用 Flux 推送',
          },
          { name: 'ConversationController', note: '会话增删改查与历史消息' },
          { name: 'UserController', note: '注册、登录、登出，BCrypt 加盐' },
          {
            name: 'FileController',
            note: '文件下载做双重防穿越：先取纯文件名，再校验规范化路径前缀',
            selfBuilt: true,
          },
        ],
      },
      {
        ...
        nodes: [
          {
            name: 'BaseAgent（抽象）',
            note: '状态机 IDLE/RUNNING/FINISHED/ERROR，管执行循环、最大步数、循环检测',
            selfBuilt: true,
          },
          { name: 'ReActAgent（抽象）', note: '把一步拆成 think 与 act 两个阶段', selfBuilt: true },
          {
            name: 'ToolCallAgent',
            note: '关闭模型内置工具执行，改由 ToolCallingManager 手动编排上下文；实现 askHuman 与 doTerminate',
            selfBuilt: true,
          },
          { name: 'MyManus', note: '具体 Agent，配置提示词与最大步数 20', selfBuilt: true },
        ],
      },
      {
        label: '能力',
        nodes: [
          { name: 'tools/（8 个 @Tool）', note: '文件、搜索、抓取、下载、PDF、询问、终止', selfBuilt: true },
          {
            name: 'chatmemory/',
            note: 'DataBaseChatMemory 把消息全量落库到 MySQL，并手工序列化工具调用响应以便跨进程重建历史',
            selfBuilt: true,
          },
          { name: 'advisors/', note: 'MyLoggerAdvisor 记录模型交互', selfBuilt: true },
          { name: 'rag/', note: 'SimpleVectorStore 本地检索，topK 3、阈值 0.5，默认关闭' },
        ],
      },
      {
        label: '支撑',
        nodes: [
          {
            ...
            note: '用 Spring AI 的 ToolCallback[] 聚合注册所有工具',
            selfBuilt: true,
          },
          {
            name: 'AgentChatOptionsConfig',
            note: '按模型提供方注入运行时 ChatOptions，实现云端与本地的零改动切换',
            selfBuilt: true,
          },
          { name: 'Service / Repository', note: '会话、消息、用户三类业务' },
        ],
      },
      {
        label: '存储',
        nodes: [
          { name: 'MySQL', note: 'user / conversation / chat_message 三张表' },
          { name: 'Redis', note: 'Spring Session 共享，多实例部署时无需改代码' },
        ],
      },
      {
        label: '扩展',
        nodes: [
          {
            ...
            note: '独立可运行的 MCP Server，基于 Pexels 图片搜索；主应用的客户端接入配置目前是注释状态',
            selfBuilt: true,
          },
        ],
      },
    ...
    decisions: [
      {
        problem:
          '需要模型自己决定何时、调用哪个工具来完成多步任务。可选路径是引入现成的 Agent 编排框架，直接用它提供的 ReAct 实现。',
        approach:
          '自己写执行循环：BaseAgent 用一个 while 循环驱动抽象 step()，ReActAgent 把 step() 定义成 think() 加 act()，ToolCallAgent 显式关闭模型的内置工具执行、改为手动编排工具调用上下文与结果回写。',
        rationale:
          '这个规模只有 8 个工具、最多 20 步，手写循环完全可控，出问题能直接断点看状态机的每一步。代码里还留着一段被注释掉的框架版 Agent 实现，说明当时确实试过框架再回退的。',
        rejected:
          '用框架的图编排能力，适合复杂 DAG，会引入节点编排与状态管理的额外复杂度，而这里需要的只是一个线性循环加终止判断。',
        tradeoff:
          '没有持久化检查点，cleanUp 只重置步数和状态、不保存执行位置，所以任务无法中断后续跑。没有并行工具分片。循环检测用的是字符串精确比对，语义相同但文字不同的重复调用检测不出来。',
      },
      {
        problem:
          '每一步都要给模型追加一条「下一步该做什么」的提示来引导它继续，但这条提示不属于用户对话，如果一起落库会污染历史记录。',
        approach:
          '在 think 阶段复制一份当前消息列表，把下一步提示作为一条 UserMessage 加进副本发给模型，发完立刻把提示字段置空，原始消息列表不被修改，因此不会被持久化。',
        rationale:
          '保持落库的历史等于真实对话内容，这样后续重新加载历史时不会出现模型自己对自己说话的奇怪上下文。',
        rejected:
          '把提示拼进 system prompt。会污染系统提示词的语义，而且随着步数增加会不断膨胀。',
        tradeoff:
          '每步多一次列表拷贝，代价很小。副作用是提示置空后只生效一次，所以循环检测触发时需要重新追加提示，这块逻辑要写两遍。',
      },
      {
        problem:
          'Agent 跑一步会产生多条消息（一条 assistant 消息加多条工具响应），要保证落库结果和内存里的历史一致，尤其是工具调用这类需要严格结构的历史。',
        approach:
          '在流式响应完成的回调里，先把该会话的历史清空，再把内存中的完整消息列表批量写入。清空用的是逻辑删除而非物理删除。同时手工把工具调用响应序列化成 JSON 存进元数据字段，保证重新加载时能被模型正确识别。',
        rationale:
          '先清空再全量写天然幂等，重复回调的结果一致，不需要维护「上次写到哪一条」的偏移量，控制流最简单。工具响应这一段是真实踩过的坑：不手工序列化的话，重新加载的历史会通不过模型方的严格校验。',
        rejected:
          '增量追加。需要额外维护偏移量与去重键，当前没有这套基础设施。',
        tradeoff:
          '代价是每次会话结束要写 O(历史长度) 条记录，历史越长越慢，而且逻辑删除会让表持续膨胀。清空与写入不在同一个事务里，中途失败会丢会话。这条也是「长会话上下文无界增长」这个问题的来源之一。',
      },
      {
        problem:
          '云端模型和本地模型会校验运行时选项的具体类型，写死一种后，切换模型提供方会直接抛类型错误。',
        approach:
          '用两个带条件装配的内部配置类，分别产出云端与本地两套运行时选项，Agent 的构造器只依赖选项接口本身。切换只需激活对应的配置档案。',
        rationale:
          'Agent 代码零改动就能在云端与本地方案之间切换，本地档案里还配了上下文长度和关闭思考模式这类实际调优参数。',
        rejected:
          '在工具调用 Agent 里写死某一家模型的选项，把现有默认实现当作兜底保留。',
        tradeoff:
          '新增模型提供方要再加一个内部类。另外这套切换能力目前只覆盖工具调用 Agent 这条链路，另一个对话应用仍直接注入模型对象，没有走这条路径。',
      },
    ],
    bottlenecks: [
      {
        title: '长会话上下文是无界增长的（已识别，尚未实现优化）',
        body: 'Agent 的内存消息列表只增不减——cleanUp 里有显式注释说明不清空历史。一次 20 步的工具任务会累积大量工具响应消息，其中可能包含完整的网页内容和 PDF 文本，下一轮请求会把这些原样再发给模型一次。结果是 token 成本随轮次线性上升，长会话还有撑爆上下文窗口的风险。我把这件事当作一个待解决的问题写在这里，而不是当成已经做好的功能：目前能定位到问题、量化和复现，方案是读取时按 token 预算裁剪加超限摘要压缩，但这部分还在设计阶段，代码里没有实现，所以我不会说它已经完成了。',
      },
      {
        title: 'MCP 只是可独立运行的示例，没有接进主应用',
        body: '图片搜索 MCP Server 是一个能单独打包运行的子模块，但主应用侧的客户端配置整段是注释状态，对应的调用方法也没有任何调用方。所以准确的说法是「提供了一个可独立运行的 MCP Server 示例」，而不是「平台支持 MCP 扩展」。',
      },
    ],
    limitations: [
      '会话接口存在越权风险：查询会话历史消息的接口没有做登录与归属校验，而同类的列表、创建、改名、删除四个接口都做了。这个漏洞我已知，是当前优先级最高的一项待修。',
      '匿名用户带着已有 chatId 对话时，会走到 userId 为空的落库路径，而会话表的 user_id 是 NOT NULL 且有外键约束，写入会在异步回调里静默失败。',
      '同一个 chatId 可以被不同用户复用，服务端在会话已存在时既不校验也不更新归属，存在跨用户消息混写的可能。',
      'docker-compose 的一键启动目前跑不通：初始化 SQL 创建的库名和后端连接的库名不一致，容器起来后表建在了另一个库里。',
      'pgvector 的向量库配置类被注释掉了，1024 维、COSINE 距离、HNSW 索引这些参数是写好了但没有启用，默认生效的是内存向量库。',
      '没有 CI。测试套件里有几个用例依赖真实的模型 Key 和 MySQL，所以整套测试跑不完整，纯单测部分只有 33 个用例。',
      '代码里残留了未使用的文件，包括一个带模拟 token 的鉴权 Advisor 和一个重读 Advisor，两者都没有任何引用。',
      '消息列表没有上限，一次多步任务的工具响应会包含完整网页内容，下一轮原样发给模型，token 成本和上下文溢出风险都随轮次上升。',
      'README 有多处与代码不一致：仓库结构、数据库名、文档目录链接（文档目录被 gitignore 排除了，GitHub 上不存在，那些链接是死链）。',
    ],
  },
  {
